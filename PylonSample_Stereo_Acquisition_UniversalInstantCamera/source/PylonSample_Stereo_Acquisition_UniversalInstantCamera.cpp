...
// Starting with Pylon 5.2, you can use the UniversalInstantCamera class to support all different interfaces at runtime (easier than using the GenAPI).
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		// Note: Some settings have different names whether the camera is USB or GigE
		if (LeftCamera.IsGigE())
			LeftCamera.ExposureTimeAbs.SetValue(c_exposureTime);
		else
			LeftCamera.ExposureTime.SetValue(c_exposureTime);
		...
		if (LeftCamera.IsGigE())
			...
			LeftCamera.ChunkSelector.SetValue(ChunkSelectorEnums::ChunkSelector_CounterValue);

		LeftCamera.ChunkEnable.SetValue(true);
		if (LeftCamera.IsGigE())
			LeftCamera.CounterSelector.SetValue(CounterSelector_Counter2);
		else
			LeftCamera.CounterSelector.SetValue(CounterSelector_Counter1);

		LeftCamera.CounterResetSource.SetValue(CounterResetSourceEnums::CounterResetSource_Software);
		...
		// image acquisition settings
		if (RightCamera.IsGigE())
			RightCamera.ExposureTimeAbs.SetValue(c_exposureTime);
		else
			RightCamera.ExposureTime.SetValue(c_exposureTime);
		...
		if (RightCamera.IsGigE())
			...
			RightCamera.ChunkSelector.SetValue(ChunkSelectorEnums::ChunkSelector_CounterValue);

		RightCamera.ChunkEnable.SetValue(true);
		if (RightCamera.IsGigE())
			RightCamera.CounterSelector.SetValue(CounterSelector_Counter2);
		else
			RightCamera.CounterSelector.SetValue(CounterSelector_Counter1);
		RightCamera.CounterResetSource.SetValue(CounterResetSourceEnums::CounterResetSource_Software);
		...
		if (LeftCamera.IsGigE() && RightCamera.IsGigE())
		{
